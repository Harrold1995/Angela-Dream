    ...
    $.extend(Plugin.prototype, {
        ...
        init: function () {
            ...
            this.$element.find('.ea-bootstrap').on('click', '.time-value', function(event) {
                ...
                if (ea_settings['pre.reservation'] === '1') {
                    ...
                    // plugin.$element.find('.step').addClass('disabled'); CHANGED
                    // plugin.$element.find('.final').removeClass('disabled'); CHANGED
        ...
        getNextOptions: function (event) {
            ...
            if (next.length === 0) {
                ...
                //nextStep.removeClass('disabled'); CHANGED
                return;
            }
        ...
        callServer: function (options, next_element) {
            ...
            $.get(ea_ajaxurl, options, function (response) {
                ...
                // enabled
                // next_element.closest('.step').removeClass('disabled'); CHANGED
        ...
        blurNextSteps: function (current, dontScroll) {
            ...
            // current.removeClass('disabled'); CHANGED
            ...
            // nextSteps.each(function (index, element) { CHANGED
            //     jQuery(element).addClass('disabled');
            // });

            // if next step is calendar
            this.selectChange(); // CHANGED
        ...
        dateChange: function (dateString, calendar) {
            ...
            // calendarEl.parent().next().addClass('disabled'); CHANGED
            ...
            jQuery.get(ea_ajaxurl, options, function (response) {
                ...
                // enabled
                // next_element.parent().removeClass('disabled'); CHANGED
                ...
                window.anpsAppointment(); // CHANGED
            }, 'json')
            .error(function(xhr, status) {
        ...
        finalComformation: function (event) {
            ...
            $.get(ea_ajaxurl, options, function (response) {
                ...
                window.anpsAppointmentDone(); // CHANGED
            }, 'json')
                .fail($.proxy(function (response, status, error) {
        ...
        cancelApp: function (event) {
            ...
            if (ea_settings['pre.reservation'] === '0') {
                plugin.chooseStep();
                plugin.res_app = null;
                // this.$element.find('.step:not(.final)').prevAll('.step').removeClass('disabled'); CHANGED
                return false;
            }

            // this.$element.find('.final').addClass('disabled'); CHANGED
            // this.$element.find('.step:not(.final)').prevAll('.step').removeClass('disabled'); CHANGED
        ...
        scrollToElement: function (element) {
            ...
            // jQuery('html, body').animate({ CHANGED
