...
class AnpsMenuWidget extends WP_Widget {
    ...
    function __construct() {
        $widget_ops  = array(
            ...
            'description' => esc_html__( 'Double column menu display. Only one level menu supported', 'hairdresser' )
        );
        $control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'anps_menu' );
        parent::__construct( 'anps_menu', esc_html__( 'AnpsThemes - Menu', 'hairdresser' ), $widget_ops, $control_ops );
    }
    ...
    public function form( $instance ) {
        ...
        if ( ! $menus ) {
            echo '<p>' . sprintf( esc_html__( 'No menus have been created yet. <a href="%s">Create some</a>.', 'hairdresser' ), admin_url( 'nav-menus.php' ) ) . '</p>';
            return;
        }

        $menu_layout_options = array(
            esc_html__('1 column', 'hairdresser') => '1-column',
            esc_html__('2 columns', 'hairdresser') => '2-column',
            esc_html__('Horizontal', 'hairdresser') => 'horizontal',
        );
        ?>

        <p><label
            for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php esc_html_e( 'Title:', 'hairdresser' ) ?></label><input
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'nav_menu' )); ?>"><?php esc_html_e( 'Select Menu:', 'hairdresser' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('menu_layout')); ?>"><?php esc_html_e("Layout", 'hairdresser'); ?></label>
